...
export function useOnlineBridge() {
  const { go, setOnline, setSession } = useAppState();
        ...
        setSession((current) =>
          current
            ? {
                ...
                guesses,
                secret,
                outcome: current.localRole === 'guesser' ? (guesserWon ? 'won' : 'lost') : guesserWon ? 'lost' : 'won',
              }
            : current,
        );
